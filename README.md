...
> 当前已实现 **仪表盘 / 服务器实例(新建 vanilla · 编辑 · 启停 · 删除)/ 实例控制台 / 系统设置**。实例目录用 UUID 命名(与显示名解耦,改名零风险)。用户/角色体系、插件管理等留作后续扩展。
...
不同 MC 版本要不同 Java(1.17→16、1.18→17、1.20.5+/1.21→21……)。在「设置 → MCDR 运行 → Java 安装池」里登记多个 `java` 路径,面板会:

- 探测每个路径的大版本并显示;
- 启动实例时按 MC 版本自动选「满足要求里最低」的那个写入该实例 `config.yml`;
- 新建对话框实时提示该版本「需 Java XX+」及是否已满足;
- 池里都不满足时,启动直接给人话错误(而非 `UnsupportedClassVersionError`)。

池为空或版本未知(快照)时,退回「默认 Java 命令」。
